...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //// Check for and
  //if (sql.find(" and ") != std::string::npos) {
  ...
  do {
      ...
      std::cout << "Words: " << substring << std::endl;
      if (substring == "or") {
  ...
  std::vector<std::string> allowedSQL = { "id=", "name=", "password=" };

  // Loop to find " or " to test for SQL Injection
  while (sql.find(" or ") != std::string::npos) {

      // Find position of " or "
      orIndex = sql.find(" or ");

      // Assign the substring to test
      std::string substring = sql.substr(orIndex + 4);
      //DEBUG: std::cout << "Substring: " << substring << std::endl;

      // Loop through vector of allowed SQL command words
      for (std::string allowedSQLCommands : allowedSQL) {

          // Check if comparing the substring to the SQL Commands is not equal
          if (allowedSQLCommands.compare(substring) != 0) {
